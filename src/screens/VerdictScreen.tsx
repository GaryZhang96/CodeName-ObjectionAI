      ...
      <div className="max-w-4xl mx-auto">
        ...
        >
          <motion.div
            ...
            className="mb-4"
          >
            <VerdictIcon className={cn('w-24 h-24 mx-auto', verdictDisplay.color)} />
          </motion.div>
          
          <h1 className={cn(
            'font-pixel-title text-4xl mb-2 glow-text',
            verdictDisplay.color
          )}>
            {verdictDisplay.text}
          </h1>
          <p className="font-pixel-title text-lg text-pixel-gray">
        ...
        >
          <Panel variant="default" className="mb-6">
            <h2 className="font-pixel-title text-pixel-gold mb-3">案件: {currentCase.title}</h2>
            <p className="font-pixel-body text-pixel-light mb-4">{verdict.reasoning}</p>
            <div className="flex items-center gap-4 text-sm">
              <span className="text-pixel-gray">陪审团最终倾向:</span>
              <span className={cn(
        ...
        {showRewards && (
          ...
          >
            <Panel variant="highlight" className="mb-6">
              <h2 className="font-pixel-title text-pixel-gold mb-4 text-center">
              ...
              <div className="grid grid-cols-2 gap-6 mb-6">
                ...
                >
                  <TrendingUp className="w-8 h-8 mx-auto text-pixel-green mb-2" />
                  <p className="font-pixel-title text-2xl text-pixel-green">
                    +{verdict.rewards.xp}
                  </p>
                  <p className="font-pixel-body text-pixel-gray">经验值</p>
                </motion.div>
                ...
                >
                  ...
                  <p className="font-pixel-title text-2xl text-yellow-400">
                    {formatMoney(verdict.rewards.money)}
                  </p>
                  <p className="font-pixel-body text-pixel-gray">报酬</p>
                </motion.div>
              </div>

              {/* 额外奖励 */}
              {verdict.rewards.bonuses.length > 0 && (
                <div className="border-t border-pixel-gold/30 pt-4">
                  <p className="font-pixel-title text-xs text-pixel-gold mb-2">额外奖励</p>
                  <div className="space-y-1">
                    {verdict.rewards.bonuses.map((bonus, i) => (
                      ...
                      >
                        ...
                        <span className="text-sm text-pixel-light">{bonus}</span>
                      </motion.div>
                    ))}
                  </div>
                </div>
              ...
              {!rewardsApplied && (
                ...
                >
                  <Button onClick={handleClaimRewards} size="lg">
                    领取奖励
                  </Button>
                </motion.div>
              )}
          ...
          >
            <Panel variant="dark" className="mb-6">
              <div className="flex items-center justify-between mb-4">
                <h2 className="font-pixel-title text-pixel-gold">
                  <Scale className="inline w-5 h-5 mr-2" />
                  复盘分析
                </h2>
                <div className="flex items-center gap-2">
                  <span className="font-pixel-title text-xs text-pixel-gray">评级:</span>
                  <span className={cn(
              ...
              <div className="grid md:grid-cols-3 gap-4">
                ...
                <div className="space-y-2">
                  <h3 className="font-pixel-title text-xs text-pixel-green flex items-center gap-1">
                  ...
                  {verdict.review.keyMoments.length > 0 ? (
                    <ul className="space-y-1">
                      {verdict.review.keyMoments.map((moment, i) => (
                        <li key={i} className="text-sm text-pixel-light flex items-start gap-2">
                    ...
                    <p className="text-sm text-pixel-gray">无特别亮眼的表现</p>
                  )}
                </div>

                {/* 失误 */}
                <div className="space-y-2">
                  <h3 className="font-pixel-title text-xs text-pixel-red flex items-center gap-1">
                  ...
                  {verdict.review.mistakes.length > 0 ? (
                    ...
                    <p className="text-sm text-pixel-gray">表现完美，无明显失误</p>
                  )}
                </div>

                {/* 改进建议 */}
                <div className="space-y-2">
                  <h3 className="font-pixel-title text-xs text-yellow-400 flex items-center gap-1">
                  ...
                  {verdict.review.improvements.length > 0 ? (
                    ...
                    <p className="text-sm text-pixel-gray">继续保持！</p>
                  )}
                </div>
              </div>
            </Panel>
        ...
        {rewardsApplied && (
          <motion.div
            ...
            className="text-center"
          >
            <Button onClick={handleContinue} size="lg" className="flex items-center gap-2 mx-auto">
              继续
